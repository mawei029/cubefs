...
func parseAndExtractSetNodeInfoParams(r *http.Request) (params map[string]interface{}, err error) {
	...
	if value = r.FormValue(cfgFlashKeyFlowLimit); value != "" {
		...
		val, err = strconv.ParseInt(value, 10, 64)
		if err != nil {
			err = unmatchedKey(cfgFlashKeyFlowLimit)
			return
		}
		params[cfgFlashKeyFlowLimit] = val
	}
...
func parseRequestToUpdateFlashTopo(r *http.Request) (args *flashTopoUpdateArgs, err error) {
	...
	if value := r.FormValue(cfgFlashNodeLruCapacity); value != "" {
		val, parseErr := strconv.Atoi(value)
		if parseErr != nil || val <= 0 {
			return nil, unmatchedKey(cfgFlashNodeLruCapacity)
		}
		args.FlashNodeLruCapacity = &val
		hasValue = true
	}
	if value := r.FormValue(cfgFlashNodeLruFhCapacity); value != "" {
		val, parseErr := strconv.Atoi(value)
		if parseErr != nil || val <= 0 || val >= 1000000 {
			return nil, unmatchedKey(cfgFlashNodeLruFhCapacity)
		}
