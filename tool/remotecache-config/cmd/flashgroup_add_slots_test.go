...
// TestToolFlashGroupListCmd_PrintGroupedSlotsWithWeight tests the list command with
// flash groups having non-zero Weight values, which triggers the totalWeight and
// expectedPct calculation in the percent display section.
func TestToolFlashGroupListCmd_PrintGroupedSlotsWithWeight(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		fgView := proto.FlashGroupsAdminView{
			FlashGroups: []proto.FlashGroupAdminView{
...
// TestToolFlashGroupSuggestSlotsCmd_WeightBasedThreshold tests suggestSlots with groups
...
func TestToolFlashGroupSuggestSlotsCmd_WeightBasedThreshold(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		if r.URL.Path == proto.AdminFlashGroupList {
			// Target FG (ID=1): Weight=3, Expected 60% but only has 2 tiny slots
			// Source FG (ID=2): Weight=2, Expected 40% but has many slots covering most space
...
// TestToolFlashGroupSuggestSlotsCmd_InactiveTarget tests suggestSlots where the target
// flash group is currently inactive (but still included in weight calculations).
func TestToolFlashGroupSuggestSlotsCmd_InactiveTarget(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		if r.URL.Path == proto.AdminFlashGroupList {
