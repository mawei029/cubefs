...
func (dataNode *DataNode) isWriteAbleWithSizeNoLock(size uint64, threshold float64) (ok bool) {
	...
	if dataNode.Total <= dataNode.Used {
		return false
	}
	remainingSpace := dataNode.Total - dataNode.Used
	if remainingSpace <= dataNode.PreReservedSpace || remainingSpace-dataNode.PreReservedSpace <= size {
		return false
	}
...
func (dataNode *DataNode) GetDecommissionFailedAndRunningDPByTerm(c *Cluster) (int, []proto.FailedDpInfo, []uint64, []uint64) {
	...
	for _, dp := range partitions {
		...
		if dp.DecommissionStatus == DecommissionRunning {
...
func (dataNode *DataNode) GetTag() string {
	return dataNode.Tag
}
