...
| aheadReadBlockSizeMB   | int64 | Size of each cache block in the sliding window (unit: MB; default: 2)         | No  |
| minReadAheadSize   | int64 | Minimum file size required to trigger the prefetch function (unit: byte; default: 10)         | No  |

After the prefetch function is enabled, for files whose size exceeds minReadAheadSize, the function will asynchronously load the content of aheadReadWindowCnt cached data blocks into the client memory, starting from the position immediately following the offset of the current read request. This effectively improves the performance of sequential reading. The size of each cached data block is configurable via aheadReadBlockSizeMB (default 2 MB).
