...
func TestFile_Setattr_BlobTruncateAndSyncReaderWriter(t *testing.T) {
	...
	patches.ApplyMethod(reflect.TypeOf(reader), "RefreshExtents", func(_ *blobstore.Reader) error { return nil })
...
func TestFile_EnsureBlobStoreWriter_CreatePath(t *testing.T) {
	...
	w, err := f.ensureBlobStoreWriter(13)
	require.NoError(t, err)
	require.NotNil(t, w)
