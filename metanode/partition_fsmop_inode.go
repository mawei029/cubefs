...
func (mp *metaPartition) checkObjExtentsConflict(mpId uint64, inoId uint64, existingExtents []proto.ObjExtentKey, newExtent, discardExtent proto.ObjExtentKey) (status uint8, finalEks []proto.ObjExtentKey) {
	...
	// loop: check if newExtent is overlap with existing extents
	...
	for i, existingEk := range existingExtents {
		...
		// No overlap with this existingEk
...
func (mp *metaPartition) fsmUpdateExtentKeyAfterMigration(inoParam *Inode) (resp *InodeResponse) {
	...
	if i.LeaseExpireTime != inoParam.LeaseExpireTime || i.Generation != inoParam.Generation {
		log.LogWarnf("fsmUpdateExtentKeyAfterMigration: inode is forbidden to migrate. leaseExpireTime %d, reqLeaseExpireTime %d, generation %d, reqGeneration %d, ino %d",
			i.LeaseExpireTime, inoParam.LeaseExpireTime, i.Generation, inoParam.Generation, i.Inode)
