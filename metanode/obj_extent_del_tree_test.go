...
func TestBatchObjExtentDelItems_MarshalEncodeErrors(t *testing.T) {
	...
	t.Run("MarshalPunish key-only round trip", func(t *testing.T) {
		...
		require.NoError(t, decoded.UnmarshalPunish(raw))
		require.Len(t, decoded.Items, 1)
		require.Equal(t, int64(1), decoded.Items[0].TsMs)
		require.Nil(t, decoded.Items[0].Oeks)
	})
...
func TestBatchObjExtentDelItems_UnmarshalPunish_keyOnly(t *testing.T) {
	var buf bytes.Buffer
	...
	require.NoError(t, batch.UnmarshalPunish(buf.Bytes()))
	require.Len(t, batch.Items, 1)
	require.Equal(t, int64(1), batch.Items[0].TsMs)
	require.Nil(t, batch.Items[0].Oeks)
}
...
func TestApplyPunishPayload_keyNotInTree_noop(t *testing.T) {
	ot := newObjExtentDelTree()
	var buf bytes.Buffer
	writeObjExtentDelV1PunishHeader(&buf, 1, 4)
	require.NoError(t, binary.Write(&buf, binary.BigEndian, int64(1)))
	require.NoError(t, binary.Write(&buf, binary.BigEndian, uint64(2)))
	require.NoError(t, binary.Write(&buf, binary.BigEndian, uint64(3)))
	require.NoError(t, ot.ApplyPunishPayload(buf.Bytes(), 1))
	require.Equal(t, 0, ot.Len())
}

func TestApplyPunishPayload_truncatedKeyBody(t *testing.T) {
	ot := newObjExtentDelTree()
	var buf bytes.Buffer
...
func TestEncodeObjExtentGcPunish_multiOek(t *testing.T) {
	...
	ot := newObjExtentDelTree()
	ot.EnqueueFromApply(2, 0, 3, []proto.ObjExtentKey{a, b})
	...
	require.Equal(t, 1, ot.Len())
	require.Len(t, ot.PeekFirstN(1).Items[0].Oeks, 2)
}
...
func TestBatchObjExtentDelItems_MarshalPunishRoundTrip(t *testing.T) {
	...
	require.Nil(t, dst.Items[0].Oeks)
}

func TestObjExtentDelPunishReplace_usesTreeOeks(t *testing.T) {
	ot := newObjExtentDelTree()
	treeOek := createTestObjExtentKey(0, 100, 1)
	ot.EnqueueFromApply(7, 100, 3, []proto.ObjExtentKey{treeOek})

	item := &objExtentDelItem{TsMs: 100 * 1000, Inode: 7, RaftIdx: 3}
	ot.mu.Lock()
	prev := ot.t.Get(item.keyItem()).(*objExtentDelItem)
	ot.objExtentDelPunishReplace(11, prev, 5000)
	ot.mu.Unlock()

	out := ot.PeekFirstN(1)
	require.Len(t, out.Items, 1)
	require.True(t, out.Items[0].Oeks[0].IsEquals(&treeOek))
}
...
func TestUnmarshalPunish_truncatedKeyBody(t *testing.T) {
	var buf bytes.Buffer
	...
	var batch batchObjExtentDelItems
	require.Error(t, batch.UnmarshalPunish(buf.Bytes()))
}
...
func TestPeekFirstDueN_skipsFuturePunishItems(t *testing.T) {
	ot := newObjExtentDelTree()
	nowMs := int64(1_700_000_000_000)
	ot.EnqueueFromApply(1, nowMs/1000, 1, []proto.ObjExtentKey{createTestObjExtentKey(0, 1, 1)})
	require.Empty(t, ot.PeekFirstDueN(10, nowMs-1).Items)
	require.Len(t, ot.PeekFirstDueN(10, nowMs).Items, 1)
}

func TestEarliestTsMs(t *testing.T) {
	require.Equal(t, int64(0), newObjExtentDelTree().EarliestTsMs())
	ot := newObjExtentDelTree()
	ot.EnqueueFromApply(1, 1700, 5, []proto.ObjExtentKey{createTestObjExtentKey(0, 1, 1)})
	ot.EnqueueFromApply(2, 1800, 6, []proto.ObjExtentKey{createTestObjExtentKey(0, 2, 2)})
	require.Equal(t, int64(1700*1000), ot.EarliestTsMs())
}
